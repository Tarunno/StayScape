      ...
      <div className='border rounded-xl shadow-xl py-6 px-6 h-full bg-white flex flex-col justify-between overflow-hidden'>
        ...
        <span className='mt-3 pt-2 flex flex-col border-t gap-2'>
          ...
          {booking.approved? <p className='flex gap-1 items-center text-green-700'>
            ...
            approved 
            <button onClick={() => handleApprove(booking._id, 'pending', customer._id, owner._id, booking.place)} className='text-[13px] ml-3 flex gap-1 items-center text-white bg-yellow-600 px-4 py-2 rounded-full'>
              pending
            </button>
            <button onClick={() => handleApprove(booking._id, 'cancel', customer._id, owner._id, booking.place)} className='text-[13px] ml-0 flex gap-1 items-center text-white bg-red-700 px-4 py-2 rounded-full'>
              Cancel
            </button>
            </p>: <p className='flex gap-1 items-center text-yellow-700'>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
            ...
            pending
            <button onClick={() => handleApprove(booking._id, 'approve',  customer._id, owner._id, booking.place)} className='text-[13px] ml-3 flex gap-1 items-center text-white bg-green-500 px-4 py-2 rounded-full'>
              Approve
            </button>
            <button onClick={() => handleApprove(booking._id, 'cancel',  customer._id, owner._id, booking.place)} className='text-[13px] ml-0 flex gap-1 items-center text-white bg-red-700 px-4 py-2 rounded-full'>
              Cancel
